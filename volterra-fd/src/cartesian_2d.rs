//! Finite-difference nematohydrodynamics on a two-dimensional Cartesian grid.
//!
//! The Landau-de Gennes molecular field, the co-rotation term, the full
//! Beris-Edwards right-hand side, a spectral Stokes solve, the Bessel-kernel
//! convolution and holonomy defect detection, all over `QField2D` and
//! `VelocityField2D`.
//!
//! This was `volterra-solver`'s crate root until that crate was dissolved. It
//! sits here because it is finite difference on a Cartesian grid, which is what
//! this crate is; the parts of `volterra-solver` written against DEC meshes
...
pub fn run_active_nematic_hydro(
    ...
    impl Observer<QField2D> for WetSink<'_> {
    ...
    let mut physics = crate::sim_impls::cartesian2d::Cartesian2DWet {
        params: params.clone(),
...
pub fn run_dry_active_nematic(
    ...
    let use_noise = params.noise_amp > 0.0;
    let mut physics = crate::sim_impls::cartesian2d::Cartesian2DDry {
        params: params.clone(),
        noise: use_noise.then(|| LangevinNoise::per_run_seed(params.nx, params.ny, n_steps)),
    };
    let mut q = q_init.clone();
...
pub fn run_bech(
    ...
    let use_noise = params.noise_amp > 0.0;
    let mut physics = crate::sim_impls::cartesian2d::Cartesian2DBech {
        params: params.clone(),
        noise: use_noise.then(|| LangevinNoise::per_run_seed(params.nx, params.ny, n_steps)),
    };
