...
fn debug_substeps() {
    ...
    // ── Step 1: H_S_from_Q ──────────────────────────────────────────────────
    h_s_from_q(
        &u, &q, &mut h, &mut s, A_LANDAU, C_LANDAU, K_ELASTIC, LAMBDA, &bnd,
    );
    ...
    eprintln!(
        "Step 3 calculate_Pi: max|Pi_S diff|={diff_pi_s:.3e}  max|Pi_A diff|={diff_pi_a:.3e}"
    );
    ...
    let ry = rhs_idx % LY;
    eprintln!(
        "Step 4a p_rhs:       max|rhs diff|={diff_rhs:.3e}  (at [{rx},{ry}] rust={:.4e} py={:.4e})",
    ...
    let pmy = pm_idx % LY;
    eprintln!(
        "Step 4b p_mid:       max|p diff|={diff_p_mid:.3e}  (at [{pmx},{pmy}] rust={:.4e} py={:.4e})",
    ...
    for i in 0..N2 {
        dq[i] = (1.0 / GAMMA) * h[i] + s[i];
    }
    upwind_advective_term(&u, &q, &mut dq, &bnd, -1.0);
    eprintln!(
        "Step 5 Q_update:     max|dQ|={:.3e}",
        dq.iter().cloned().fold(0.0_f64, f64::max)
    );
    ...
    eprintln!(
        "Step 6 u_update:     max|dudt|={:.3e}",
        dudt.iter().cloned().fold(0.0_f64, f64::max)
    );
    ...
    volterra_fd::bc::apply_q_boundary_conditions(&mut q_new, &bnd, std::f64::consts::SQRT_2, 1.0);
    volterra_fd::bc::apply_u_boundary_conditions(&mut u_new, &bnd);
    ...
    eprintln!(
        "Final: max|Q diff|={diff_q_final:.3e}  max|u diff|={diff_u_final:.3e}  max|p diff|={diff_p_final:.3e}"
    );
