//! A whole CGPO step, device resident.
//!
//! [`DeviceState`] holds every field `volterra_cgpo::step::State` holds, on the
//! device, and [`DeviceState::step`] runs one advance of
//! `volterra_cgpo::step::update_step_inner` across them. Nothing crosses the
...
/// The physical parameters a step needs, mirroring `volterra_cgpo::Params`.
...
impl StepParams {
    ...
    pub fn from_cpu(p: &volterra_cgpo::Params) -> Self {
        Self {
            a_landau: p.a_landau,
            c_landau: p.c_landau,
...
impl Device {
    ...
    ///
    /// Follows `volterra_cgpo::step::update_step_inner` stage for stage, and
