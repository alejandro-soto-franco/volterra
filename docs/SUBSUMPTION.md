...
| One-constant LdG bulk (`A`), no B/C cubic term, `chi=1` flow-alignment (paper's name; code's `lambda`), Frank constant `K=2^14` | flow-solver.py / Klein et al. | `volterra-fd2d::Params` (`volterra-fd2d/src/lib.rs:34-133`) | covered: bit-for-bit kernel concurrence against this exact reference, `COMPARISON.md` section 1 |
...
| No-slip velocity at a domain wall | open-zetar (implicit; open-zetar has no walls, only periodic) | `apply_u_boundary_conditions`, DEC (`volterra-dec/src/boundary_conditions.rs`) and the finite-difference path (`volterra-fd2d/src/bc.rs:101-116`) | covered, exceeds open-zetar (which has no wall BC at all, periodic only) |
| Tangential strong (Dirichlet) anchoring, winding-index-parameterised, on a circular boundary (Eq. 1, arXiv:2503.10880) | flow-solver.py | `volterra-fd2d::bc::apply_q_boundary_conditions` with `net_charge` (`volterra-fd2d/src/bc.rs:242-281`), boundary geometry `boundary::circular_boundary` (`boundary.rs:299`) | covered, **added during this dispatch**; not previously present, not validated against a captured Python reference the way the nephroid path was (see `docs/REPLICATION.md`) |
...
| Epitrochoid family: cardioid, nephroid, trefoiloid confinement with tangential anchoring | flow-solver.py | `volterra-dec::epitrochoid::{disk_mesh,epitrochoid_mesh}` (`volterra-dec/src/epitrochoid.rs:115-302`, DEC solver) and, separately, `volterra-fd2d::boundary::nephroid_boundary` (FD solver, `boundary.rs:168`) | covered, by two independent, non-interoperating volterra solvers rather than one |
...
| File-loaded initial configuration (`--initialConfigurationFile`) | open-Qmin | `--theta-ic` (`volterra/src/cli.rs:170-172,482-517`) | partial: file-loaded IC exists, but only for the `fd2d` subcommand's scalar theta grid, not the Cartesian/DEC Q-tensor solvers |
...
| Random per-site theta on `[0, pi)`, masked to the interior | flow-solver.py | `volterra-fd2d/src/bin/fd2d.rs` random-theta IC path (mirrors `flow-solver.py`'s masked random init) | covered |
...
| Explicit Euler, upwind advection, capped pressure-Poisson relaxation (`max_p_iters`), `dt=1e-4` | flow-solver.py | `volterra-fd2d::step` (`volterra-fd2d/src/step.rs`), driven by `fd2d` | covered, bit-for-bit kernel concurrence |
...
| Plain-text `%.18e` frames, numpy-`savetxt`-compatible layout | flow-solver.py | `volterra-fd2d::output::{write_2col_txt,write_1col_gauge_fixed}` (`output.rs:29,46`) | covered, bit-for-bit format match |
