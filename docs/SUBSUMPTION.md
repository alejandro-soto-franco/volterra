...
| Epitrochoid family: cardioid, nephroid, trefoiloid confinement with tangential anchoring | flow-solver.py | `volterra-dec::epitrochoid::{disk_mesh,epitrochoid_mesh}` (`volterra-dec/src/epitrochoid.rs:115-302`, DEC solver) and, separately, `volterra-fd::boundary::nephroid_boundary` (FD solver, `boundary.rs:168`) | covered, by two independent, non-interoperating volterra solvers rather than one |
