...
| Bulk phase constants a, b, c (cubic B term in the bulk potential) | open-Qmin | `ActiveNematicParams3D::b_landau` (`volterra-core/src/lib.rs:332`) | partial: volterra's 3D bulk has a cubic term (`b_landau`), its 2D bulk does not (`volterra-core/src/lib.rs:71`, no `b` field) |
