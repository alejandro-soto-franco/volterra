...
| E-field and H-field coupling, including a spatially varying field loaded from file | open-Qmin | `ActiveNematicParams3D::{chi_a, b0, omega_b}` for the magnetic field and `{epsilon_a, e0, omega_e}` for the electric one, both entering `mol_field_3d::field_term` | partial: both couplings are present as uniform fields, and each may rotate in the xy plane; a *spatially varying* field loaded from file is still absent, which is the remaining part of this row |
...
| One-constant LdG bulk (`A`), no B/C cubic term, `chi=1` flow-alignment (paper's name; code's `lambda`), Frank constant `K=2^14` | flow-solver.py / Klein et al. | `volterra-fd::Params` (`volterra-fd/src/lib.rs:34-133`) | covered: bit-for-bit kernel concurrence against this exact reference, `COMPARISON.md` section 1 |
...
| No-slip velocity at a domain wall | open-zetar (implicit; open-zetar has no walls, only periodic) | `apply_u_boundary_conditions`, DEC (`volterra-dec/src/boundary_conditions.rs`) and the finite-difference path (`volterra-fd/src/bc.rs:101-116`) | covered, exceeds open-zetar (which has no wall BC at all, periodic only) |
| Tangential strong (Dirichlet) anchoring, winding-index-parameterised, on a circular boundary (Eq. 1, arXiv:2503.10880) | flow-solver.py | `volterra-fd::bc::apply_q_boundary_conditions` with `net_charge` (`volterra-fd/src/bc.rs:242-281`), boundary geometry `boundary::circular_boundary` (`boundary.rs:299`) | covered, **added during this dispatch**; not previously present, not validated against a captured Python reference the way the nephroid path was (see `docs/REPLICATION.md`) |
...
| Epitrochoid family: cardioid, nephroid, trefoiloid confinement with tangential anchoring | flow-solver.py | `volterra-fd::boundary::{epitrochoid_boundary,cardioid_boundary,nephroid_boundary,trefoiloid_boundary}` (FD solver) and, separately, `volterra-dec::epitrochoid::{disk_mesh,epitrochoid_mesh}` (DEC solver) | covered on the FD path for the whole family, by two independent, non-interoperating volterra solvers rather than one. flow-solver.py fixes the cusp count at 2 and the regularisation at `d=0.99`; the FD path takes both as parameters, so it runs the cardioid and the trefoiloid the reference script cannot. The DEC path is still `d=1`, the sharp epicycloid, and so is not the geometry the paper simulates |
...
| File-loaded initial configuration (`--initialConfigurationFile`) | open-Qmin | `--theta-ic` (`volterra/src/cli.rs:170-172,482-517`) | partial: file-loaded IC exists, but only for the `fd` subcommand's scalar theta grid, not the Cartesian/DEC Q-tensor solvers |
...
| Random per-site theta on `[0, pi)`, masked to the interior | flow-solver.py | `volterra-fd/src/bin/fd.rs` random-theta IC path (mirrors `flow-solver.py`'s masked random init) | covered |
...
| Explicit Euler, upwind advection, capped pressure-Poisson relaxation (`max_p_iters`), `dt=1e-4` | flow-solver.py | `volterra-fd::step` (`volterra-fd/src/step.rs`), driven by `fd` | covered, bit-for-bit kernel concurrence |
...
| Stress tensor (`Pi_S`, `Pi_A`), computed internally, never exported from a runner | flow-solver.py, open-zetar | none exported | **gap, additive**: volterra's finite-difference stress kernels compute the same quantities internally but no runner returns them as an observable, matching the reference's own limitation rather than exceeding it; exporting an already-computed field is not a redesign |
...
| Plain-text `%.18e` frames, numpy-`savetxt`-compatible layout | flow-solver.py | `volterra-fd::output::{write_2col_txt,write_1col_gauge_fixed}` (`output.rs:29,46`) | covered, bit-for-bit format match |
...
| numba `parallel=True`/`prange` CPU threading across the whole grid (`os.cpu_count() - 1` threads) | open-zetar | rayon `.into_par_iter()`, but only on the 3D **dry** path (`mol_field_3d.rs:77-251`) | partial: volterra's 2D Cartesian solver, 3D wet/BECH path, the entire DEC engine, and `volterra-braid` are single-threaded; only the 3D dry path and the finite-difference row-chunked kernels are parallel |
| Single-threaded, numba JIT compiled | flow-solver.py | `volterra-fd`'s row-chunked rayon kernels, which switch to rayon above a 250,000-cell threshold (`par_gate.rs:29,62-78`) | covered, exceeds flow-solver.py (measured 4.6x single-thread-vs-32-core-numba speedup, `COMPARISON.md` section 2), single-threaded below the threshold |
