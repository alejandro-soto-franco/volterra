...
//!
//! [`DeviceState`] holds every field `volterra_fd2d::step::State` holds, on the
//! device, and [`DeviceState::step`] runs one advance of
//! `volterra_fd2d::step::update_step_inner` across them. Nothing crosses the
...
/// The physical parameters a step needs, mirroring `volterra_fd2d::Params`.
...
pub struct StepParams {
    ...
    /// `volterra-fd2d/examples/jacobi_census.rs` measures the golden
...
impl StepParams {
    ...
    pub fn from_cpu(p: &volterra_fd2d::Params) -> Self {
        Self {
            a_landau: p.a_landau,
            c_landau: p.c_landau,
...
impl Device {
    ...
    ///
    /// Follows `volterra_fd2d::step::update_step_inner` stage for stage, and
