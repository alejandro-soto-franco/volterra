...
directory:
  ...
  * run ONE (active_length_scale, nematic_coherence_length) point from env
    (FD2D_ALS / FD2D_NCL) instead of the 4x5 sweep;
  * grid size and step budget from env (FD2D_LX/FD2D_LY/FD2D_MAX_STEPS/FD2D_MAX_T)
...
Then (smoke):   FD2D_LX=60 FD2D_MAX_STEPS=2000 uv run --with numpy --with numba flow_solver_run.py
Then (full golden, once the als/ncl map is fixed):  FD2D_ALS=.. FD2D_NCL=.. uv run --with numpy --with numba flow_solver_run.py
"""
...
PATCHES = [
    ...
    ("for active_length_scale in [1, 2, 3, 4]:",
     "for active_length_scale in [int(os.environ.get('FD2D_ALS', '2'))]:  # patched: single point"),
    ("    for nematic_coherence_length in [4, 5, 6, 7, 8]:",
     "    for nematic_coherence_length in [int(os.environ.get('FD2D_NCL', '6'))]:  # patched: single point"),
    # grid + step budget from env (for smoke runs)
    ("Lx = 200  # number of lattice sites along x direction",
     "Lx = int(os.environ.get('FD2D_LX', '200'))  # patched: env-overridable"),
    ("Ly = 200  # number of lattice sites along y direction",
     "Ly = int(os.environ.get('FD2D_LY', '200'))  # patched: env-overridable"),
    ("max_steps = 1500000",
     "max_steps = int(os.environ.get('FD2D_MAX_STEPS', '1500000'))  # patched"),
    ("max_t = 1500000",
     "max_t = int(os.environ.get('FD2D_MAX_T', '1500000'))  # patched"),
