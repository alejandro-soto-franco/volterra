//! Smoke test for `volterra run fd2d`.
//!
//! Spawns the binary on a tiny 16x16 grid for 2 steps with snap_every=1 and
//! asserts exit 0 plus the expected frame output structure.

use std::process::Command;

#[test]
fn run_fd2d_exits_zero_and_writes_frame_files() {
    let tmp = std::env::temp_dir().join("vcli_fd2d_smoke");
    ...
    assert!(status.success(), "volterra run fd2d exited non-zero: {status}");
...
#[test]
fn run_fd2d_writes_final_frame_when_steps_not_multiple_of_snap_every() {
    ...
    let tmp = std::env::temp_dir().join("vcli_fd2d_off_cadence");
    let _ = std::fs::remove_dir_all(&tmp);
