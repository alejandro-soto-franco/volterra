//! Smoke test for `volterra run fd`.
//!
...
#[test]
fn run_fd_exits_zero_and_writes_frame_files() {
    let tmp = std::env::temp_dir().join("vcli_fd_smoke");
...
#[test]
fn run_fd_writes_final_frame_when_steps_not_multiple_of_snap_every() {
    ...
    let tmp = std::env::temp_dir().join("vcli_fd_off_cadence");
    let _ = std::fs::remove_dir_all(&tmp);
