...
fn defaults_out_dir_per_subcommand() {
    let cli = Cli::try_parse_from(["volterra", "run", "fd2d"]).expect("parse");
    if let Command::Run(RunTarget::Fd2d(a)) = cli.command {
        assert_eq!(
            a.common.out_or_default("fd2d"),
            std::path::PathBuf::from("./output/fd2d")
        );
    } else {
        panic!("expected fd2d");
    }
