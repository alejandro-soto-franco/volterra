...
//!
//! - `validate`: each operator against `volterra_fd::ops`, on a random field
...
use volterra_fd::ops;
use volterra_fd_cuda::{Device, DeviceBoundary, DeviceState, StepParams};
...
/// `volterra_fd::ops::upwind_advective_term` with each accumulation written
...
fn upwind_reference_fma(
    ...
    arr: &[f64],
    out: &mut [f64],
    bnd: &volterra_fd::boundary::Boundary,
    coeff: f64,
) {
...
fn phase_validate() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The Stokes tranche, on the smooth Q and the fields it produces.
    {
...

        {
            ...
            volterra_fd::stokes::calculate_pressure_terms(
...

        {
            ...
            volterra_fd::stokes::relax_pressure_inner_loop(&mut cpu, &p_aux, &rhs, &bnd);
...

        {
            ...
            volterra_fd::stokes::get_u_update(
    ...
    // interior mask.
    {
...

        {
            ...
            volterra_fd::bc::apply_u_boundary_conditions(&mut cpu, &bnd);
        ...
        for charge in [1.0_f64, 1.5, 2.0] {
            ...
            volterra_fd::bc::apply_q_boundary_conditions(&mut cpu, &bnd, params.s0, charge);
...

        {
            ...
            volterra_fd::bc::apply_p_boundary_conditions(
...
fn phase_step(steps: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut cpu = volterra_fd::step::State::new(LX, LX);
    ...
    for step in 0..steps {
        let cpu_iters =
            volterra_fd::step::update_step_inner(&mut cpu, &params, &bnd, target_rel_change);
...
///
/// The CPU side is `volterra_fd` as the braid runs use it, which at 100x100
...
fn phase_time(steps: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fresh = || {
        let mut st = volterra_fd::step::State::new(LX, LX);
        let mut rng = StdRng::seed_from_u64(0);
    ...
    // Warm up both, so neither pays a first-touch cost inside the timing.
    {
        let mut w = fresh();
        for _ in 0..64 {
            volterra_fd::step::update_step_inner(&mut w, &params, &bnd, target);
        }
    ...
    for _ in 0..steps {
        volterra_fd::step::update_step_inner(&mut cpu, &params, &bnd, target);
    }
...
fn phase_batch(runs: usize, steps: usize) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // The comparison a sweep deserves. `volterra_fd` is single threaded at
    ...
    std::thread::scope(|scope| {
        for r in 0..runs {
            ...
            scope.spawn(move || {
                ...
                for _ in 0..steps {
                    volterra_fd::step::update_step_inner(&mut st, &cpu_params, bnd, target);
                }
            });
        }
    });
...
/// `fd` writes, so the published braid can be extracted from them.
///
...
fn phase_golden(
    ...
    let bnd = boundary::circular_boundary(LX, LX);
    let d_bnd = DeviceBoundary::upload_full(dev.stream(), &bnd)?;
    let params = volterra_fd::Params::new(LX, 3.99, 0.975, 1.0, 1e-4, 50).with_net_charge(charge);
    let step_params = StepParams::from_cpu(&params);
    let target = 1e-3;
