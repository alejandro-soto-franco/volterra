//! Beris-Edwards with a Stokes flow on a genus-2 surface.
//!
//! A sphere owes four `+1/2` defects by Poincare-Hopf. A genus-2 surface owes
//! a total charge of `-2`, so it must carry NEGATIVE defects, four `-1/2` at
//! the least. That inversion is the reason for this driver: the sphere can say
//! nothing about it.
//!
//! The surface is a tube around a Bernoulli lemniscate, marched and then
//! remeshed until the cotangent weights are non-negative. Its curvature is
//! negative over most of it, so the outer Stokes factor `Delta + 2K` is shifted
//! the opposite way from the sphere, and it has no Killing field for the solve
//! to project out.
//!
//! Writes the same files as `sphere_braid`, so every reader downstream works
//! unchanged.
...
fn main() {
    ...
    let mut grid = 40_usize;
    let mut tube = 0.24_f64;
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--tube" => {
                i += 1;
                tube = args[i].parse().expect("invalid --tube value");
            }
            "--grid" | "-r" => {
                i += 1;
                grid = args[i].parse().expect("invalid --grid value");
            }
            "--t-final" => {
                i += 1;
    ...
    println!("  Pe = {pe}");
    println!("  grid = {grid}, tube radius = {tube}");
    println!("  dt = {dt}");
    ...
    println!("Marching the genus-2 tube (grid {grid}, r = {tube})...");
    let (gverts, gtris) = genus2_mesh(tube, grid, 6);
    let mesh = cartan_dec::mesh::Mesh::from_simplices(
        &Euclidean::<3>,
        gverts.iter().map(|p| nalgebra::SVector::from(*p)).collect(),
        gtris,
    );
    ...
    let domain = DecDomain::new(mesh, Euclidean::<3>).expect("DecDomain assembly failed");
    ...
    let mut fdef =
        std::fs::File::create(out.join("defects.csv")).expect("failed to open defects.csv");
    writeln!(fdef, "step,t,x,y,z,charge").unwrap();
    let mut fstat = std::fs::File::create(out.join("stats.csv")).expect("failed to open stats.csv");
    writeln!(
        fstat,
        "step,t,n_plus,n_minus,mean_S,total_charge,u_rms,pe_measured"
    )
    .unwrap();
    ...
    for step in 0..=n_steps {
        if step % snap_every == 0 {
            ...
            let defs = detect_defects_surface(
                &stokes_coords,
                &domain.mesh.simplices,
                &domain.mesh.boundaries,
                &domain.mesh.simplex_boundary_ids,
                &edge_phases,
                &q,
            );
            for (p, c) in &defs {
                writeln!(
                    fdef,
                    "{step},{t_sim:.6},{:.6},{:.6},{:.6},{c}",
                    p[0], p[1], p[2]
                )
                .unwrap();
            }
            let npl = defs.iter().filter(|d| d.1 > 0).count();
            // The total is Poincare-Hopf: `-2` here, where a sphere owes `+2`.
            // It is recorded rather than asserted so a run that breaks it is
            // diagnosable afterwards rather than merely dead.
            let pe_measured = u_rms / (params.gamma_r * params.k_r);
            writeln!(
                fstat,
                "{step},{t_sim:.6},{npl},{},{:.6},{},{u_rms:.6},{pe_measured:.4}",
                defs.len() - npl,
                q.mean_order_param(),
                total_charge(&defs)
            )
        ...
        if step % (snap_every * 5) == 0 {
            ...
            let per_step = if step > 0 {
                cg_iters as f64 / step as f64
            } else {
                0.0
            };
            println!(
                "  t={t_sim:6.2}/{t_final}  step {step:>7}/{n_steps}  <S>={s:.4}  wall={elapsed:.1}s  ({rate:.0} steps/s, {per_step:.0} cg/step)"
            );
        }
