...
fn the_vorticity_is_the_laplacian_of_the_stream_function() {
    ...
    let psi: Vec<f64> = (0..nv)
        .map(|i| (k * coords[i][0]).sin() * (k * coords[i][1]).sin())
        .collect();
    ...
    let on_wall: std::collections::HashSet<usize> = cm.boundary_vertices.iter().copied().collect();
    let mut num = 0.0_f64;
    let mut den = 0.0_f64;
    let mut n = 0usize;
    for i in 0..nv {
    ...
    assert!(
        rel < 0.25,
        "vorticity is off by {rel:.3e} relative, sign or scaling wrong"
    );
    // and the sign specifically: the two must be anticorrelated with psi
    let dot: f64 = (0..nv)
        .filter(|i| !on_wall.contains(i))
        .map(|i| w[i] * psi[i])
        .sum();
    assert!(
        dot < 0.0,
        "Delta psi should oppose psi for this mode, got dot = {dot:.3e}"
    );
}
