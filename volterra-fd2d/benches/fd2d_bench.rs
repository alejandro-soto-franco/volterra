...
//! solver.
//!
//! One benchmark, `fd2d_step`, advances the solver a few steps on a small grid
//! (lx=32) through the same public surface `fd2d` uses: a `Fd2dStep`
...
//!
//! Run: `cargo bench -p volterra-fd2d --bench fd2d_bench`
...
/// Random theta IC at interior cells, matching `fd2d::random_theta_ic`.
...
fn fd2d_step(c: &mut Criterion) {
    let lx = 32usize;
    let ly = lx;
    let n_steps = 5usize;

    // Same parameter map fd2d uses (defaults als=2.8, ncl=4.8, lambda=1).
...
criterion_group!(benches, fd2d_step);
