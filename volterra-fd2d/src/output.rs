//! Shared output helpers for frame writing.
//!
//! Both the `volterra` CLI and the `fd2d` binary write frames through
...
use crate::{boundary::Boundary, error::{Fd2dError, Fd2dResult}, step::State};
...
///
/// The pressure Poisson solve uses pure Neumann BCs, so the absolute
...
/// Write one frame (Q, u, p) into `run_dir` under `Q/`, `u/`, `p/`.
///
/// Creates subdirectories as needed. Returns `Fd2dError::Io` on any failure.
///
...
pub fn write_state_frame(
    ...
    write_2col_txt(&q_path, &state.q, n)
        .map_err(|source| Fd2dError::Io { path: q_path.clone(), source })?;
    write_2col_txt(&u_path, &state.u, n)
        .map_err(|source| Fd2dError::Io { path: u_path.clone(), source })?;
    write_1col_gauge_fixed(&p_path, &state.p, &boundary.inside, n)
        .map_err(|source| Fd2dError::Io { path: p_path.clone(), source })?;
