...
use crate::error::{Fd2dError, Fd2dResult};
...
pub fn check_cfl(
    ...
    if dt > safe {
        return Err(Fd2dError::Cfl { step, dt, safe, umax });
    }
    Ok(())
}

/// Verify every entry of `field` is finite; error names the first offending step/field.
pub fn check_finite(field: &[f64], name: &'static str, step: usize) -> Fd2dResult<()> {
    if field.iter().any(|v| !v.is_finite()) {
        return Err(Fd2dError::NonFinite { step, field: name });
    }
    Ok(())
}
