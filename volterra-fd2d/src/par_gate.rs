...
//!
//! - `FD2D_FORCE_PARALLEL=1`: always use rayon regardless of grid size
//! - `FD2D_FORCE_SERIAL=1`: always use serial regardless of grid size
...
fn get_override() -> Override {
    *OVERRIDE.get_or_init(|| {
        if std::env::var("FD2D_FORCE_PARALLEL").as_deref() == Ok("1") {
            Override::ForceParallel
        } else if std::env::var("FD2D_FORCE_SERIAL").as_deref() == Ok("1") {
